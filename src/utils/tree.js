...
class Tree {
  ...
  formatNodes() {
    ...
    // 设置每一列的位置
    // 每列两边留出20px的空白
    const leftWidthArr = this.leftTree.map(tree => Math.max(...tree.map(node => node.width)) + 40);
    const rightWidthArr = this.rightTree.map(tree => Math.max(...tree.map(node => node.width)) + 40);
